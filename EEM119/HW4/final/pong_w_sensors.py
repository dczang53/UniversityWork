...
class Ball():
	...
	def reset(self, direction):
		...
		if direction == "Right":
			self.vel = [random.randint(4,8),-random.randint(2,6)]
		elif direction == "Left":
			self.vel = [-random.randint(4,8),-random.randint(2,6)]
...
def new_game(window_obj):
	...
	while option == "nil":
		...
		for event in pygame.event.get():
			...
			pygame.display.update()
			fpsClock.tick(25)						#run at 60 fps
...
def game_loop(window_obj, ball, paddle1, paddle2, two_player):
	while True:												#main game loop
			...
			try:
				...
				#print(addr)
				if addr[0] == sensor_IP:
					distance = struct.unpack("f" * 2, data)
					#print(distance[0])
					#print(distance[1])
					if distance[0] <= 25:
						paddle1.pos[1] = int(HEIGHT-paddle1.height/2) - 1
					elif distance[0] >= 75:
						paddle1.pos[1] = int(paddle1.height/2) + 1
					else:
						paddle1.pos[1]= int(HEIGHT-paddle1.height/2) - int(((HEIGHT-paddle1.height) / 50) * (distance[0] - 25))
					if two_player:
						if distance[1] <= 25:
							paddle2.pos[1] = int(HEIGHT-paddle2.height/2) - 1
						elif distance[1] >= 75:
							paddle2.pos[1] = int(paddle2.height/2) + 1
						else:
							paddle2.pos[1]= int(HEIGHT-paddle2.height/2) - int(((HEIGHT-paddle2.height) / 50) * (distance[1] - 25))
...
#NOTES:
#input the IP address of the Raspberry Pi controlling both sensors as first and only argument
#can get IP address using "hostname -I"

#SOURCES:
#https://github.com/maaverik/Pong/blob/master/Pong.py
