...
def new_game(window_obj):
	...
	while option == "nil":
		...
		for event in pygame.event.get():
			...
			pygame.display.update()
			fpsClock.tick(20)						#run at 60 fps
	game_loop(window_obj, ball, paddle1, paddle2, option)

'''
def paddle_sensor(paddles):
	sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	sock.bind((socket.gethostbyname("0.0.0.0"), 8080))
	print("instantiated read thread")
	while game_on:
		...
		if addr in paddles:
			distance = float(data)
			if distance <= 25:
				paddles[addr].pos[1] = int(paddles[addr].height/2) + 1
			elif distance >= int(HEIGHT-paddles[addr].height/2):
				paddles[addr].pos[1] = int(HEIGHT-paddles[addr].height/2) - 1
			else:
				paddles[addr].pos[1]= int(paddles[addr].height/2) + int(((HEIGHT-paddles[addr].height) / 50) * (distance - 25))
...
def game_loop(window_obj, ball, paddle1, paddle2, two_player):
	...
	sock.bind((socket.gethostbyname("0.0.0.0"), 8080))
	print("instantiated read thread")
	#receiver = threading.Thread(target=paddle_sensor, args=(ip_to_paddle,))
	#receiver.start()
	while True:												#main game loop
			...
			try:
				...
				print("addr")
				print(addr)
				if addr[0] in player_IP:
					distance = struct.unpack('f', data)[0]
					if distance <= 25:
						ip_to_paddle[addr[0]].pos[1] = int(ip_to_paddle[addr[0]].height/2) + 1
					elif distance >= int(HEIGHT-ip_to_paddle[addr[0]].height/2):
						ip_to_paddle[addr[0]].pos[1] = int(HEIGHT-ip_to_paddle[addr[0]].height/2) - 1
					else:
						ip_to_paddle[addr[0]].pos[1]= int(ip_to_paddle[addr[0]].height/2) + int(((HEIGHT-ip_to_paddle[addr[0]].height) / 50) * (distance - 25))
...
if(len(sys.argv) != 3):
	...
	sys.exit(1)
player_IP.append(sys.argv[1])
player_IP.append(sys.argv[2])
print(player_IP)
new_game(window_obj)
