...
template<typename ItemType>
Sequence<ItemType>::Sequence()          // Create an empty sequence (i.e., one whose size() is 0).
{createEmpty();}

template<typename ItemType>
bool Sequence<ItemType>::empty() const  // Return true if the sequence is empty, otherwise false.
{return size() == 0;}

template<typename ItemType>
int Sequence<ItemType>::size() const    // Return the number of items in the sequence.
{return m_size;}
