...
//        vector<byte> data;
        ...
        {
            throw runtime_error("We failed to read the second SYN packet.");
        }
        else
        {
            ...
            Packet step_three_syn_packet{get_then_increment_next_expected_packet(1),
    ...
    void wait_for_syn_packet()
    {
		...
		//tuple<int, int, int, bool, bool, bool, vector<byte>>
		//step-1 looking for client
		while (1)
		{
			...
			if (syn_flag)
			{
				...
				next_sequence_number = random_number_distribution(random_number_generator);
				next_expected_packet = sequence_number;
				int current_sequence_number = sequence_number;
				...
				while ((status = poll(rfd, 1, 0)) >= 0)
				{
					if (rfd[0].revents & POLLIN)
					{
						...
						if (ack_flag && (acknowledgement_number == next_sequence_number))
						{
							//how to get data? dunno how to convert byte to char
	...
	//not sure of incrementing seguence number
	void close_connection()    //after receiving FIN from client
	{
		struct pollfd rfd[1];
		rfd[0].fd = my_socket_file_descriptor;
		rfd[0].events = POLLIN;
		...
		//poll for possible retransmitted FIN / if timeout, shutdown
		int status;
		while ((status = poll(rfd, 1, 0)) >= 0)
		{
			if (rfd[0].revents & POLLIN)
			{
				auto fin_packet{ wait_for_packet() };
				auto[sequence_number, acknowledgement_number, receive_window, ack_flag, syn_flag, fin_flag, file_data] = fin_packet.value();
				if (fin_flag && (sequence_number==next_sequence_number))
				{
					send_acknowledgement(0);
					restart_clock();
					cout << "Sending packet " << next_sequence_number << 0 << endl;
...
int main(int argc, char *argv[])
{
    if (argc < 2)
    {
        throw runtime_error("You need to give a port number.");
    }


    TCP_Connection my_connection{"2401"};

	my_connection.wait_for_syn_packet();    //complete 3 way handshake and stores filename for later

	//what if ACK for very last packet is not received on server end?
	//will packets keep on being sent until it receives a fin packet?

	my_connection.close_connection();    //use immediately when last FIN from client is received

	/*
	basically:
	-server last packets can be considered FIN packets
	-after client sends its last ACK correspondin to which packet the FIN bit is on
	-client sends its ACK packet (to FIN) in a loop, until server receives it
	-server receives FIN packet while still sending packets on loop (probably all FINs at this point)
	-server must detect the FIN packet while polling on loop, and return ack (since we don't know if it received packet before FIN)
	-client sends FIN on loop
	-server gets FIN, and sends back ACK (if timeout after 2RTT, shutdown)
	-client, after receiving ACK, shutdown

	(basically second half, with roles reversed)
	
	following: http://www.tcpipguide.com/free/t_TCPConnectionTermination-2.htm
	*/



	/*
    auto test{my_connection.get_data()};

    for (auto moo : test)
    {
        cout << static_cast<char>(moo) << endl;
    }
	*/
}
